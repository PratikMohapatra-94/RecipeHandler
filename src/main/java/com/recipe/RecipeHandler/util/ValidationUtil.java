...
public class ValidationUtil {
	...
	public static String validateRqst(RecipeRqstPL recipeRequest) {
		if (recipeRequest != null) {
			...
			} else if (recipeRequest.getDishName().length() < 3 || recipeRequest.getDishName().length() > 100) {
			...
			} else if (recipeRequest.getSuggestedServings() > 50 || recipeRequest.getSuggestedServings() < 1) {
				return "Enter valid number of servings between 1 - 49!";
			}
			if (recipeRequest.getInstructions() == null || recipeRequest.getInstructions().trim() == "") {
				return "Enter recipe Instructions!";
			} else if (recipeRequest.getInstructions().length() < 3 || recipeRequest.getInstructions().length() > 1000) {
